<template>
  <header class="bg-gray-900 pt-4yarn w-full z-20 sticky top-0">
    <nav
      ...
      <nuxt-link to="/" class="flex justify-center lg:col-span-1 h-2/3">
        ...
        <nuxt-img provider="sanity" :src="Logo.imageId.asset._ref" alt="logo vr café" class="h-full" sizes="xs:100vw" />
      </nuxt-link>
      ...
      <!-- navigation -->
      <div class="hidden col-span-2 lg:col-span-1 lg:col-start-2 lg:row-start-1 lg:flex flex-row gap-4 items-center">
        <div v-for="lien in liens" :key="lien.texte" class="mb-4 py-4 flex space-x-6">
          <a key="Index" :href="lien.lien" class="text-base lg:text-2xl font-medium text-white hover:text-indigo-50"
            @click="toggle">
            {{ lien.texte }}
          </a>
        </div>
    ...
    <!-- nav ouverte -->
    <nav :class="{ hidden: isActive }"
      class="lg:hidden w-screen absolute top-0 h-screen z-40 bg-gray-900 grid grid-cols-2 grid-rows-6 ">
      ...
      <div v-for="lien in liens" :key="lien.texte" class="mb-4 py-4 flex flex-col items-center">
        <nuxt-link key="Index" :to="lien.lien" class="text-2xl font-medium text-white hover:text-indigo-50"
          @click.native="toggle">
          {{ lien.texte }}
        </nuxt-link>
      </div>
